# Cookbook

Small beginner recipes for common game features. Each page points at a matching
`examples/*` crate that you can copy into a new project.

Use `templates/data-driven-demo` or `examples/data-driven-full-demo` when you
want to stay in `assets/game.ron`; use the recipes below when you are ready for
beginner Rust builder chains.

- [Coins and score](coins-and-score.md)
- [Projectiles](projectiles.md)
- [Two levels](two-levels.md)
- [Enemy waves](enemy-waves.md)
- [Triggers and areas](triggers-and-areas.md)
- [Audio](audio.md)
- [UI and menus](ui.md)
- [Gamepad controls](gamepad.md)
- [LDtk levels](ldtk.md)
- [Win conditions](win-conditions.md)
- [Enemy drops](enemy-drops.md)
- [Health pickups](health-pickups.md)
- [Checkpoints](checkpoints.md)
- [Bosses](bosses.md)
- [Dialog](dialog.md)
- [Inventory and status](inventory.md)
- [Title menu](title-menu.md)
- [Damage zones](damage-zones.md)
- [Common small-game patterns](common-small-game-patterns.md)
- [Menu and game over](menu-and-game-over.md)
- [Animations](animations.md)
- [Custom tags and timers](custom-tags-and-timers.md)
- [Live tuning](live-tuning.md)
