...
- [ ] legacy `game-dev validate-data assets/game.ron` checked
...
- [ ] no-Rust SDK artifacts generated and verified
- [ ] tutorial sequence checked
- [ ] roadmap/status docs updated
- [ ] distribution policy checked

Primary no-Rust release gates:

- [ ] primary no-Rust template contains no Rust files
- [ ] primary no-Rust package runs without `cargo`
- [ ] primary authoring files pass Rust-vocabulary scanner
- [ ] primary docs do not teach Rust/RON as the default path
- [ ] prebuilt runner loads `game.toml`
- [ ] packaged artifact verification includes no-Rust project
...
## Prebuilt Demo And SDK Release Artifacts

Tags matching `v*` run `.github/workflows/release.yml`. The workflow packages
the bundled demo and the no-Rust SDK for Linux x86_64 and Windows x86_64,
uploads workflow artifacts, and attaches these zips to the GitHub Releases
page:
...
Demo zips contain the playable demo executable, any bundled runtime libraries,
`assets/`, launcher scripts, and `README.txt` at the archive root. SDK zips
contain `game-player`, `game-dev`, `templates/no-rust-demo/`, launchers,
`README.txt`, `LICENSE`, `THIRD_PARTY_NOTICES.md`, and optional no-Rust example
packages.

The workflow builds with `ci-build-sdl3` so the prebuilt artifacts do not depend
on runner-provided SDL3 packages. Keep the
...
For a local Linux dry-run of the no-Rust SDK contents:

```bash
cargo run -p xtask --features ci-build-sdl3 -- package-sdk --release --features ci-build-sdl3 --out /tmp/game-sdk-linux-x86_64
( cd /tmp/game-sdk-linux-x86_64 && zip -r ../game-sdk-linux-x86_64.zip . )
...
cd /tmp/my-game
/tmp/game-sdk-linux-x86_64/game-dev check
/tmp/game-sdk-linux-x86_64/game-dev preview --smoke-frames 60
```
...
That release predates the SDK artifacts. For releases after the no-Rust SDK
workflow landed, verify both `game-demo-*` and `game-sdk-*` zips.
