...
mod tests {
    ...
    const TEST_TEXTURE: TextureHandle = TextureHandle(0);
    const FONT_TEXTURE: TextureHandle = TextureHandle(1);

    fn sprite(texture: TextureHandle, layer: i16) -> SpriteDraw {
        SpriteDraw {
            texture,
            layer,
    ...
    fn build_into_merges_runs_and_sorts_by_layer() {
        ...
        assert!(batch.push(sprite(TEST_TEXTURE, 0)));
        assert!(batch.push(sprite(FONT_TEXTURE, 5)));
        assert!(batch.push(sprite(TEST_TEXTURE, 0)));
        ...
        assert_eq!(ranges[0].texture, TEST_TEXTURE);
        assert_eq!(ranges[0].first_vertex, 0);
        assert_eq!(ranges[0].vertex_count, 12);
        assert_eq!(ranges[1].texture, FONT_TEXTURE);
    ...
    fn build_into_appends_with_absolute_offsets_across_batches() {
        ...
        assert!(world.push(sprite(TEST_TEXTURE, 0)));
        let mut ui = SpriteBatch::new();
        assert!(ui.push(sprite(FONT_TEXTURE, 0)));
    ...
    fn build_into_layer_order_wins_over_texture_id() {
        ...
        let mut batch = SpriteBatch::new();
        assert!(batch.push(sprite(TEST_TEXTURE, 5)));
        assert!(batch.push(sprite(FONT_TEXTURE, 0)));
        ...
        assert_eq!(ranges.len(), 2);
        assert_eq!(ranges[0].texture, FONT_TEXTURE);
        assert_eq!(ranges[1].texture, TEST_TEXTURE);
    }
    ...
    fn clear_resets_submission_order_and_drop_count() {
        ...
        let mut invalid = sprite(TEST_TEXTURE, 0);
        invalid.size.x = 0.0;
