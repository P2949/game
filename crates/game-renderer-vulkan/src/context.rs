...
impl VulkanContext {
    ...
    fn submit_render_frame(&mut self, frame: RenderFrame) {
        for sprite in frame.world_sprites {
        ...
        for text in frame.ui_text {
            self.draw_ui_text_layer(&text.text, text.pos, text.color, text.layer);
        }
    }

    fn draw_ui_text_layer(&mut self, text: &str, pos: glam::Vec2, color: glam::Vec4, layer: i16) {
...
impl DrawCommands for VulkanContext {
    ...
    fn draw_ui_text(&mut self, text: &str, pos: glam::Vec2, color: glam::Vec4) {
        self.draw_ui_text_layer(text, pos, color, UI_TEXT_LAYER);
    }
}
