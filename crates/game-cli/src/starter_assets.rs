...
const BUILTIN_FONT: &[u8] = include_bytes!("../../../assets/fonts/DejaVuSans.ttf");

pub(crate) fn write_starter_assets(assets: &Path) -> Result<()> {
    ...
    for sound in [
        ...
        if !path.exists() {
            fs::write(&path, make_wav(sound))
                .with_context(|| format!("failed to write '{}'", path.display()))?;
        }
    }

    write_builtin_font(assets)?;
    Ok(())
}

pub(crate) fn write_builtin_font(assets: &Path) -> Result<()> {
    let fonts = assets.join("fonts");
    fs::create_dir_all(&fonts)
        .with_context(|| format!("failed to create '{}'", fonts.display()))?;
    let path = fonts.join("DejaVuSans.ttf");
    if !path.exists() {
        fs::write(&path, BUILTIN_FONT)
            .with_context(|| format!("failed to write '{}'", path.display()))?;
    }
    Ok(())
}
